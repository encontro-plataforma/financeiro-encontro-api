...
def list_encontreiros(
    ...
    equipe_ids: list[int] | None = Query(default=None),
    situacao_camisa: list[SituacaoCamisa] | None = Query(default=None),
...
def update(
    ...
    return EncontreiroService.update(
        db, encontreiro_id, data.model_dump(exclude_none=True)
    )


@router.patch(
    "/{encontreiro_id}/equipe/{equipe_id}", response_model=EncontreiroResponse
)
