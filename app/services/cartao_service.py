...
def _montar_observacao(linha: CartaoLinhaDTO) -> str:
    forma_label = "Crédito" if linha.forma_pagamento.value == "CARTAO_CREDITO" else "Débito"
    return (
...
class CartaoService:
    ...
    def processar_em_background(upload_id: int, conteudo: str):
        ...
        try:
            ...
            logger.exception("Erro ao processar extrato de cartão (upload_id=%s)", upload_id)
            UploadFileService.update_status(
                db,
                upload_id,
