...
**Important nuance for Banco Inter statements**: `BancoInterParser` appends the row's account balance ("Saldo") to `observacao` before hashing. Two genuinely distinct payments (same payer, same day, same value — e.g. an inscription paid in two installments) would otherwise collide on hash and get wrongly rejected as duplicates; the balance always differs between them, so folding it into `observacao` differentiates the hash without needing a new column. Same-file exact duplicates (identical row repeated, including balance) still collide and surface as a row-level error, not a silent skip.
...
| `APP_VERSION`       | `0.3.1`                                                         | App version, shown in `/health`                                                                     |
